...
const bmpEncoder = require("bmp-js");
...
module.exports = async function (context, req) {
  ...
  if (!storageConnectionString) {
    ...
    context.res = {
      status: 500,
      body: {
        error: "Server configuration error: Missing storage connection string.",
      },
    };
    return;
  }
  ...
  try {
    ...
    if (action === "getUploadSas") {
      ...
      const blobName =
        req.query.filename ||
        `${Date.now()}-${Math.random().toString(36).substring(2, 15)}.tmp`; // Generate a unique name if not provided
      ...
      context.log(
        `Successfully generated SAS URL (HTTP/HTTPS) for upload to ${containerName}/${blobName}`
      );
      context.res = {
        status: 200,
        body: {
      ...
      if (!req.body || !req.body.sourceBlobName || !req.body.formats) {
        context.res = {
          ...
          body: {
            error:
              "sourceBlobName and formats are required in the request body.",
          },
        };
        return;
      }
      ...
      context.log(
        `Downloading source image: ${inputContainerName}/${sourceBlobName}`
      );
      const sourceBlobClient =
        inputContainerClient.getBlobClient(sourceBlobName);
      ...
      try {
        ...
        context.log(
          "Source image downloaded successfully from uploads container."
        );
      } catch (downloadError) {
        context.log.error(
          `Failed to download source image ${sourceBlobName} from ${inputContainerName}: ${downloadError.message}`
        );
        // Check if the sourceUrl was provided as a fallback (e.g., if client uploaded elsewhere)
        if (
          req.body.sourceUrl &&
          typeof req.body.sourceUrl === "string" &&
          req.body.sourceUrl.startsWith("https://")
        ) {
          context.log.warn(
            `Attempting to download from provided sourceUrl: ${req.body.sourceUrl}`
          );
          const response = await fetch(req.body.sourceUrl);
          if (!response.ok) {
            throw new Error(
              `Failed to download source image from provided URL: ${response.statusText}`
            );
          }
          const arrayBuffer = await response.arrayBuffer();
          imageBuffer = Buffer.from(arrayBuffer);
          context.log(
            "Source image downloaded successfully using provided sourceUrl."
          );
        } else {
          throw new Error(
            `Failed to download source image and no valid fallback sourceUrl provided. Error: ${downloadError.message}`
          );
        }
      }
      ...
      for (const formatKey of Object.keys(formats)) {
        if (formats[formatKey]) {
          ...
          context.log(
            `Processing format: ${formatKey} (Target: ${targetDimensions.width}x${targetDimensions.height})`
          );
          ...
          if (formatKey.endsWith(".bmp")) {
            processedBuffer = await createMonochromeBmp(
              imageBuffer,
            ...
            }; // Assuming target is met
          } else {
            // Assuming PNG otherwise
            const pngResult = await createPng(
              imageBuffer,
              targetDimensions.width,
              targetDimensions.height,
              context
          ...
          const outputBlobName = `${Date.now()}_${formatKey.replace(
            /[^a-zA-Z0-9.]/g,
            "_"
          )}`; // Sanitize name
          const outputBlobClient =
            outputContainerClient.getBlockBlobClient(outputBlobName);

          context.log(
            `Uploading processed image: ${outputBlobName} as ${contentType}`
          );
          ...
          context.log(
            `Processed and uploaded: ${formatKey} (Actual: ${finalInfo.width}x${finalInfo.height}) to ${outputBlobClient.url}`
          );
        }
      }
      ...
      context.res = {
        status: 400,
        body: {
          error: "Invalid action. Must be 'getUploadSas' or 'processImage'.",
        },
      };
    }
  } catch (error) {
...
function getFormatDimensions(format, context) {
  ...
  context.log.warn(
    `Dimensions not found for format: ${format}. Using default 300x300.`
  );
